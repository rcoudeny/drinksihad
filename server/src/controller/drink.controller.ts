...
export class DrinkController {
    ...
    async getDrink(@CurrentUser({ required: true }) user: UserDTO, @Param('drinkId') drinkId: string, @Param('userMail') userMail: string): Promise<UserDrinkDTO> {
        return DrinkMapper.toUserDrinkDTO(await DrinkService.getDrink(drinkId, (await UserService.getUserWithEmail(userMail)).id));
    }
